...
class OpencartController(http.Controller):
	...
	def opencart_webhook_json(self, storeId, **kwargs):
		...
		if 'eventType' not in request.jsonrequest.keys():
			return False
		print(request.jsonrequest)
		eventType = request.jsonrequest['eventType']
		if eventType != 'order.updated':
			return False


		orderstatus_new = request.jsonrequest['orderstatus_new']
		orderId = request.jsonrequest['orderId']
		no_action = request.jsonrequest['no_action']
		...
		if order_id:
			...
			if not no_action:
				...
				if status_id.status in ['CANCELLED']:
					print("___CANCELLED")
					order_id.with_context(no_send_status_update=True).action_cancel()
