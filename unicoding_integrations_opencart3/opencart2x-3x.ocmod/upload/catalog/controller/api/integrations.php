    ...
    public function syncproducts(){
        ...
        if (!isset($this->session->data['api_id'])) {
            ...
            if (!isset($parse["scheme"])){
                ...
                $url = $parse["scheme"] . "://" . $parse["host"] . ($parse["port"] ? ":" . $parse["port"] : "");
        ...
        if($route = "checkout/order/addOrderHistory" && $input_data && isset($input_data[0])) {
            ...
            if ((int)$this->config->get('config_odoo_status_order') ){
                ...
                if ($order_info['order_status_id'] != $order_state_id){
                    ...
                    $fields = array("eventType" => "order.updated", "orderId"=>$order_id, "orderstatus_new"=>$order_state_id, "no_action"=>($order_comment=="odoo") ? True : False);
