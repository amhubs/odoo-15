...
class UnicodingOpencartStatus(models.Model):
    ...
    status = fields.Selection([
        ...
        Refunded - When you give back order  from client and everything is ok with goods, after you must return money to customer, order must be set to Refunded. In odoo thats mean Invoice Order status will be change to Refunded and system create Refuned Invoice.\n
        Complete - If the order has arrived at its destination and Paid it can be set to complete. In Odoo thats meanse that Delivery status is Done, Invoice status is Paid.""" )
